...
void create_item(const string& nama_barang, const string& ukuran, const string& harga) {
    ...
    if (conn) {
        string query = "INSERT INTO rajeg_store (nama_barang, ukuran, harga) VALUES ('" + nama_barang + "', '" + ukuran + "', '" + harga + "')";
...
void get_items() {
    MYSQL* conn = connect_db();
    if (conn) {
        if (mysql_query(conn, "SELECT * FROM rajeg_store")) {
...
void update_item(int item_id, const string& nama_barang, const string& ukuran, const string& harga) {
    ...
    if (conn) {
        stringstream query;
        query << "UPDATE rajeg_store SET nama_barang = '" << nama_barang << "', ukuran = '" << ukuran << "', harga = '" << harga << "' WHERE id = " << item_id;
...
void delete_item(int item_id) {
    ...
    if (conn) {
        stringstream query;
        query << "DELETE FROM rajeg_store WHERE id = " << item_id;
        if (mysql_query(conn, query.str().c_str())) {
